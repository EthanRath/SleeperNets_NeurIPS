...
def make_env(env_id, seed, idx, capture_video, run_name, atari, highway):
    def thunk():    
        if atari:
...
if __name__ == "__main__":
    ...
    for e in environs:
        ...
        envs = gym.vector.SyncVectorEnv(
            [make_env(args.env_id, args.seed + i, i, args.capture_video, run_name, args.atari, args.highway) for i in range(args.num_envs)]
        )
        assert isinstance(envs.single_action_space, gym.spaces.Discrete), "only discrete action space is supported"

        q_network = QNetwork(envs, not (args.safety or args.trade), args.safety, args.trade).to(device)
        optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate)
        target_network = QNetwork(envs, not (args.safety or args.trade), args.safety, args.trade).to(device)
